...
function Minion(spriteTexture, atX, atY) {
    this.kSpeedX = 10 + Math.random() * 20;
    this.kSpeedY = 10 + Math.random() * 20;
    ...
    if (Math.random() > 0.5) {
        this.kSpeedX = -this.kSpeedX;
    }
    if (Math.random() > 0.5) {
        this.kSpeedY = -this.kSpeedY;
    }
    r.setVelocity([this.kSpeedX, this.kSpeedY]);
...
Minion.prototype.onCollisionEnter = function (otherObj) {
    if (otherObj.mName === "Main")
    {
        var v = this.getPhysicsComponent().getVelocity();
//        v[1] = -v[1];
    }
    else if (otherObj.mName === "Brick")
    {
        var v = this.getPhysicsComponent().getVelocity();
        v[1] = -v[1];
    }
    console.log("Collision by %s ", otherObj.mName);
};