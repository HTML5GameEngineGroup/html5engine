...
MyGame.prototype.loadScene = function () {
    ...
    gEngine.Textures.loadTexture(this.s2);
};

MyGame.prototype.unloadScene = function () {
    gEngine.Textures.unloadTexture(this.kMinionSprite);
    gEngine.Textures.unloadTexture(this.kMinionPortal);
    gEngine.Textures.unloadTexture(this.s2);
};
...
MyGame.prototype.initialize = function () {
    ...
    this.mPortal.getXform().incSizeBy(3);
...
MyGame.prototype.update = function () {
    var msg = "Hz=" + this.mHero.getXform().mZ + 
              "  Pz=" + this.mPortal.getXform().mZ;
...
*/
    ...
    if (gEngine.Input.isKeyClicked(gEngine.Input.keys.One)) {
        this.mLMinion.change1();
    }

    if (gEngine.Input.isKeyClicked(gEngine.Input.keys.Two)) {
        this.mLMinion.change2();
    }
    
    if (gEngine.Input.isKeyPressed(gEngine.Input.keys.Z)) {
        if (gEngine.Input.isKeyPressed(gEngine.Input.keys.H)) {
            this.mHero.getXform().mZ += 0.1;
            msg += " Hz=" + this.mHero.getXform().mZ;
        }
        if (gEngine.Input.isKeyPressed(gEngine.Input.keys.B)) {
            this.mPortal.getXform().mZ += 0.1;
            msg += " Bz=" + this.mPortal.getXform().mZ;
        }
    }
    ...
    if (gEngine.Input.isKeyPressed(gEngine.Input.keys.X)) {
        ...
        if (gEngine.Input.isKeyPressed(gEngine.Input.keys.B)) {
            this.mPortal.getXform().mZ -= 0.1;
            msg += " mPortal=" + this.mPortal.getXform().mZ;
        }
    }
