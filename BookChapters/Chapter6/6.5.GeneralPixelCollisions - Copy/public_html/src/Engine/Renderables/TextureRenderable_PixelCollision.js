TextureRenderable.prototype.PixelTouches = function(other, wcTouchPos) {
    ...
    var xIndex = this._mTexLeftIndex;
    var otherIndex = [0, 0];
    ...
    while ((!pixelTouch) && (xIndex<=this._mTexRightIndex)) {
        var yIndex = this._mTexBottomIndex;
        while ((!pixelTouch) && (yIndex<=this._mTexTopIndex)) {
            if (this.GetTexelAlpha(xIndex, yIndex) > 0) {
                ...
                if ((otherIndex[0] >= other._mTexLeftIndex) && (otherIndex[0] <= other._mTexRightIndex) &&
                    (otherIndex[1] >= other._mTexBottomIndex) && (otherIndex[1] <= other._mTexTopIndex)) {
